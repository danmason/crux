...
|===
|Name|Transaction Log|Topology|Java Class
|<<#config-standalone, Standalone>>|Uses local event log|`:crux.standalone/topology`|`StandaloneTopology`
|<<#config-kafka, Kafka>>|Uses Kafka|`:crux.kafka/topology`|`KafkaTopology`
|<<#config-jdbc, JDBC>>|Uses JDBC event log|`:crux.jdbc/topology`|`JDBCTopology`
|===
...
|===
|Property|Java Function|Default Value
|`:crux.node/kv-store`|`withKvStore`|*`'crux.kv.rocksdb/kv`*
|`:crux.node/object-store`|`withObjectStore`|*`'crux.object-store/kv-object-store`*
|===
...
[#table-conversion%header,cols="d,d,d,d"]
|===
|Property|Java Function|Description|Default Value
|`:crux.kv/db-dir` | `withDbDir` |Directory to store K/V files | *data*
|`:crux.kv/sync?`| `withSync` |Sync the KV store to disk after every write?| *false*
|`:crux.kv/check-and-store-index-version` | `withCheckAndStoreIndexVersion` |Check and store index version upon start? | *true*
|===
...
|===
|Property|Java Function|Description|Default Value
|`:crux.standalone/event-log-kv-store` |`withEventLogKvStore`|Key/Value store to use for standalone event-log persistence | *'crux.kv.rocksdb/kv*
|`:crux.standalone/event-log-dir`|`withEventLogDir`|Directory used to store the event-log and used for backup/restore, i.e. `"data/eventlog-1"`|
|`:crux.standalone/event-log-sync?` |`withEventLogSync`|Sync the event-log backend KV store to disk after every write? | *false*
|===
...
|===
|Property|Java Function|Description|Default value
|`:crux.kafka/bootstrap-servers`|`withBootstrapServers`|URL for connecting to Kafka|*localhost:9092*
|`:crux.kafka/tx-topic`|`withTxTopic`|Name of Kafka transaction log topic|*crux-transaction-log*
|`:crux.kafka/doc-topic`|`withDocTopic`|Name of Kafka documents topic|*crux-docs*
|`:crux.kafka/create-topics`|`withCreateTopics`|Option to automatically create Kafka topics if they do not already exist|*true*
|`:crux.kafka/doc-partitions`|`withDocPartitions`|Number of partitions for the document topic|*1*
|`:crux.kafka/replication-factor`|`withReplicationFactor`|Number of times to replicate data on Kafka|*1*
|`:crux.kafka/group-id`|`withGroupId`|Kafka client group.id|*(Either environment variable _HOSTNAME_, _COMPUTERNAME_, or a random UUID)*
|`:crux.kafka/kafka-properties-file`|`withKafkaPropertiesFile`|File to supply Kakfa connection properties to the underlying Kafka API|
...
[#table-conversion%header,cols="d,d,d"]
|===
|Property|Java Function|Description
|`:crux.kv/db-dir`|`withDbDir`|_For h2 and sqlite_
|`:crux.jdbc/host`|`withHost`|Database Host
|`:crux.jdbc/user`|`asUser`|Database Username
|`:crux.jdbc/password`|`withPassword`|Database Password
|===
...
demonstration image to experiment with the <<#rest,REST API>>.

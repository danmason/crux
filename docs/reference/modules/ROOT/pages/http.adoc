...
Crux offers a REST API layer in the `crux-http-server` module that allows you to send transactions and run queries over HTTP. (See the REST API reference xref:http-api.adoc[here])
...
All of the REST endpoints return both `application/edn` and `application/transit+json` (see transit https://github.com/cognitect/transit-clj[here]) - individual endpoints may return additional types - see the docs below. To set up your node to with the REST API, see the xref:http.adoc[HTTP Feature Module].
...
.*Required Parameters*
* `eid` (Crux ID)
* `sort-order` (_required_, either `asc` or `desc`)
...
curl -g \
     -X GET \
     -H "Content-Type: application/edn" \
     $CRUX_URL/_crux/query?q={%3Afind+[e]+%3Awhere+[[e+%3Acrux.db/id+_]]}
----

==== Query Parameters

.*Required Parameters*
* `q` (URL Encoded datalog query)
